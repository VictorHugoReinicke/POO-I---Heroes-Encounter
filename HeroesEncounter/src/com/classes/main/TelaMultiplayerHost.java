...
public class TelaMultiplayerHost extends JDialog {
    ...
    private void iniciarHost() {
        if (networkManager.startAsHost()) {
            ...
            new Thread(this::aguardarConexao).start();
        } else {
            JOptionPane.showMessageDialog(this, "Erro ao iniciar servidor!", "Erro", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void aguardarConexao() {
        Socket client = networkManager.waitForClient();
        if (client != null) {
    ...
    private void iniciarJogo() {
        if (jogadorHost == null || jogadorConvidado == null) {
            JOptionPane.showMessageDialog(this, "Selecione ambos os personagens!", "Aviso", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        TelaAventuraMultiplayer telaAventura = new TelaAventuraMultiplayer(jogadorHost, jogadorConvidado, networkManager, true);
