...
public class TelaAventura extends JFrame {
	...
	private void initializeTela() {
		...
		// ✅ PAINEL DE BOTÕES ATUALIZADO - AGORA COM 6 BOTÕES
		...
		adicionarLog("❤️ HP: " + jogador.getHp() + "/" + jogador.getHpMax());
	...
	private void iniciarJornada() {
		if (!jornadaAtiva) {
			...
			adicionarLog("==========================================");
			adicionarLog("⚔️ JORNADA INICIADA! ⚔️");
			adicionarLog("==========================================");
	...
	private Inimigo obterInimigoAleatorio(boolean isChefe) {
		try {
			...
			if (inimigosFiltrados.isEmpty()) {
				// Se não encontrou o tipo específico, usar qualquer inimigo disponível
				adicionarLog("⚠️ Nenhum " + (isChefe ? "chefe" : "inimigo normal")
			...
			// Fallback de emergência - criar inimigos básicos
			if (isChefe) {
				...
				// Criar um inimigo normal aleatório
			...
			// Último fallback absoluto - isso não deveria acontecer
